...
public abstract class LocationServiceImplementation implements LocationRepository
{
	 ...
	 * } //Modify Location
	 * 
	 * @Override public Location modifyLocation(Location location, int locationId) {
	 * Optional<Location> a = locationRepository.findById(locationId);
	 * if(a.isPresent()) { Location modified = a.get();
	 * modified.setLocationId(location.getLocationId());
	 * modified.setName(location.getName()); modified.setCode(location.getCode());
	 * modified.setCountry(location.getCountry());
	 * modified.setAirportName(location.getAirportName());
	 * LocationRepository.save(modified); return modified; } return null; }
	 */
		
	/*
	 * //Delete Location
	 * 
	 * @Override public Location viewLocationBy(String id) throws
	 * LocationNotFoundException { Location
	 * add=LocationRepository.findLOcationById(id); if(add==null) throw new
	 * LocationNotFoundException("Location Not Found "); return add; }
	 */
